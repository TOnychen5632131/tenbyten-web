    ...
    return (
        <div className="flex flex-col items-center justify-center py-4">
            ...
            <button
                ...
                className="group relative flex items-center gap-3 px-6 py-3 rounded-full transition-all duration-300 active:scale-95 hover:bg-white/5"
            >
                ...
                <span className="relative z-10 text-2xl md:text-3xl font-light tracking-tight text-white font-sans">
                    {selectedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    <span className="opacity-50 ml-2 font-normal text-lg">{selectedDate.getFullYear()}</span>
                </span>

                <ChevronDown className={`relative z-10 text-white/40 transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${showCalendar ? 'rotate-180' : ''}`} size={24} strokeWidth={1.5} />
            </button>
